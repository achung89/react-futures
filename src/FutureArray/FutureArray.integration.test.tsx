...
const PropDrill = ({prop, level}) => {
  ...
  const Drilled = [...Array(level)].reduce( Comp => {
...
describe("integration scenarios", () => {
  test("should render ", async () => {
    let App = ({ nestedFuture = false }) => {
      ...
      if(nestedFuture) {
        ...
        numbers = numbers.map( (num, ind) => {
          console.log(num)
          return num + numbers2[ind]
        }) // [9,8,7,9]
        .filter( num => {
          console.log(num);
          return new StubFutureArray(8) // [1,2,3,8]
  ...
  test('should allow intermediate transformations', () => {
