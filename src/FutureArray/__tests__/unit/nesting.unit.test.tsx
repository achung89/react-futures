...
describe('Nested Future array in lazy array callback', () => {
  it('should render simple array', async () => {
    ...
    let renderer;
    act(() => {
      renderer = render(<Suspense fallback={<div>Loading...</div>}><App /></Suspense>, container);
    });
    const { getByText } = renderer;
    jest.runOnlyPendingTimers();
    await waitFor(() => getByText('Loading...'));
  ...
  it('should render simple nested array', async () => {
    ...
    let renderer;
    act(() => {
      renderer = render(<Suspense fallback={<div>Loading...</div>}><App nestedFuture /></Suspense>, container);
    });
    const { getByText } = renderer;
    jest.runOnlyPendingTimers();
    await waitFor(() => getByText('Loading...'));
...
const createSimpleNestedFuture = numbers => numbers.map((num, i) => num + new FutureArr(5)[i])
